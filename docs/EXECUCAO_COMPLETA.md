...
python -m pip install -r apps/ingest/api/requirements.txt
python -m pip install -r apps/ingest/api/benchmarks/sciq/requirements-benchmark.txt
```
...
docker compose -f apps/ingest/docker-compose.yml up -d --build
docker compose -f apps/ingest/docker-compose.yml ps
...
Saída: `benchmark/runs/novas/sciq/sciq-atualizada-01/`.
...
Saída: `benchmark/runs/novas/pdf-ir/pdf-principal-01/`.
...
Saída: `benchmark/runs/novas/pdf-ir/pdf-chunking-gold-01/`. Além dos agregados de cada
...
```bash
python -m pip install -r apps/chat/api/requirements.txt
```
...
python benchmark/run_experiment.py rag \
  ...
  --cases benchmark/runs/novas/pdf-ir/pdf-principal-01/recursive_text/ragas-groundtruth.jsonl \
  --collection freire_recursive_gold_50_01 \
  --methods bm25,dense,hybrid \
  --top-k 5 \
  --evaluate-ragas
...
Saída: `benchmark/runs/novas/rag/rag-principal-01/`. São até 150 respostas geradas
