...
[`apps/ingest/api/benchmarks/qasper/README.md`](../apps/ingest/api/benchmarks/qasper/README.md).
...
```bash
python3 benchmark/run_experiment.py sciq \
...
python3 benchmark/run_experiment.py pdf-ir --help
python3 benchmark/run_experiment.py rag --help
```

Cada execução acadêmica usa um diretório timestampado em
`benchmark/runs/novas/<estudo>/` e grava um `manifest.json`. As rodadas anteriores
ficam congeladas em `benchmark/runs/antigos/`. Os comandos detalhados abaixo
...
```text
apps/ingest/api/benchmarks/sciq/data/runs/YYYYMMDD-HHMMSS/
...
python3 benchmark/groundtruth/materialize.py materialize \
  --cases benchmark/data/freire/cases.jsonl \
  --chunks benchmark/data/freire_recursive/chunks.jsonl \
  --ingest-out benchmark/data/freire_recursive/ingest-groundtruth.jsonl \
  --ragas-out benchmark/data/freire_recursive/ragas-groundtruth.jsonl \
  --report-out benchmark/data/freire_recursive/matching-report.json
```
...
cd apps/ingest/api
INGEST_APP_PASSWORD=alterar-esta-senha python benchmarks/groundtruth/run_groundtruth.py \
  --cases ../../benchmark/data/freire_recursive/ingest-groundtruth.jsonl \
  --base-url http://localhost:8010 \
  --collection freire_recursive
```
...
```text
apps/ingest/api/benchmarks/groundtruth/data/runs/YYYYMMDD-HHMMSS/
├── results.jsonl
├── metrics.csv
└── summary.json
```
...
```text
benchmark/data/freire_recursive/ragas-groundtruth.jsonl
```
...
CHAT_APP_PASSWORD=alterar-esta-senha python benchmarks/ragas/run_groundtruth.py \
  --cases ../../benchmark/data/freire_recursive/ragas-groundtruth.jsonl \
...
```text
apps/chat/api/benchmarks/ragas/data/runs/YYYYMMDD-HHMMSS/
├── results.jsonl
├── responses.csv
├── summary.json
