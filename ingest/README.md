...
Este repositório separa deliberadamente `ingest/` do futuro `chat/`. O módulo atual recebe PDFs, extrai o conteúdo com Docling, divide o documento em chunks dinâmicos, armazena o original no MinIO, os metadados/chunks no MySQL e vetores dense+sparse no Qdrant.
...
Na primeira subida, o `sentence-transformers/all-MiniLM-L6-v2`, o modelo sparse e os artefatos do Docling podem ser baixados para o volume persistente `model_cache`. Isso requer acesso à internet e pode levar alguns minutos; nas próximas inicializações eles são reutilizados.
...
Por padrão, a extração usa Docling para converter PDFs em uma representação textual/Markdown mais rica que a extração simples por página. O chunking é dinâmico: o texto é dividido por blocos e parágrafos usando os defaults do código, sem reiniciar o corte a cada página.
...
- Aceita PDFs que o Docling consiga converter para texto. PDFs escaneados podem exigir OCR/modelos adicionais e mais tempo de processamento.
