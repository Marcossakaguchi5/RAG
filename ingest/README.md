...
Para uma bateria manual por terminal, use `ingest/api/benchmarks/groundtruth/`. Ela recebe um JSONL com `query` e `relevant_chunk_ids`, roda `bm25`, `dense` e `hybrid` contra a API do ingest e salva `results.jsonl`, `metrics.csv` e `summary.json` em uma pasta com timestamp.
