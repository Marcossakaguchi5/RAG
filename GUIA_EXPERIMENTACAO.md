...
| **QASPER** | benchmark adicional de RI em artigos longos | parágrafos textuais anotados como evidência | pipeline própria em `ingest/api/benchmarks/qasper/` |
...
O ponto de entrada acadêmico é `experiments/run_experiment.py sciq`. Ele executa
o pipeline interno, cria uma pasta nova em `experiments/runs/novas/sciq/<run-id>/`
e registra revisão do dataset, modelo, chunking, seed, commits e versões. O
comando de baixo nível `ingest/api/benchmarks/sciq/run_all.py` continua útil para
...
`chat/api/app/services/reranker.py` usa somente a biblioteca padrão do Python:
...
O fluxo em `experiments/groundtruth/` separa julgamento científico e associação
técnica:
...
use `experiments/run_experiment.py pdf-ir` ou
`ingest/api/benchmarks/groundtruth/run_groundtruth.py`: eles recebem qrels
...
## 7. Estrutura de `experiments/`

| Caminho | Responsabilidade |
| --- | --- |
| `experiments/run_experiment.py` | ponto de entrada para rodadas `sciq`, `pdf-ir` e `rag`; cria diretório e manifesto |
| `experiments/analyze_sciq.py` | IC95% e contrastes pareados da rodada SciQ |
| `experiments/plot_pdf_ir.py` | tabelas, SVGs, ICs, curvas e contrastes do PDF/RI |
| `experiments/groundtruth/` | auditoria do PDF e materialização estrita de citações para qrels |
| `experiments/cases/` | casos mestres; `draft` é piloto e `gold` é a versão revisada |
| `experiments/runs/novas/` | destino de novas rodadas, sem sobrescrever resultados |
| `experiments/runs/antigos/` | registros históricos congelados |
...
```text
experiments/runs/novas/<estudo>/<run-id>/
...
```bash
python experiments/run_experiment.py sciq --run-id sciq-01 \
  --qdrant-url http://localhost:6335 \
  --methods bm25,dense,hybrid

python experiments/run_experiment.py pdf-ir --run-id pdf-ri-01 \
...
- [PLANO_EXPERIMENTAL.md](PLANO_EXPERIMENTAL.md): questões de pesquisa, escopo e
  ameaças à validade.
- [experiments/README.md](experiments/README.md): visão rápida da pipeline.
- [experiments/groundtruth/README.md](experiments/groundtruth/README.md): formato,
  normalização e falhas do materializador.
- [BENCHMARKS.md](BENCHMARKS.md): comandos e benchmarks disponíveis.
- [chat/api/benchmarks/ragas/README.md](chat/api/benchmarks/ragas/README.md): coleta
