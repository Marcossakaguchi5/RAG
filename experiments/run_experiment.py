...
INGEST_API = ROOT / "ingest" / "api"
CHAT_API = ROOT / "chat" / "api"
DEFAULT_RUNS = ROOT / "experiments" / "runs" / "novas"
CHUNKING_STRATEGIES = {
    "fixed_token",
    "recursive_text",
...
def run_sciq(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.statistics:
            statistics_command = [
                sys.executable,
                str(ROOT / "experiments" / "analyze_sciq.py"),
                "--run-dir",
                str(run_dir),
                "--data-dir",
...
def run_pdf_ir(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        source_audit_command = [
            sys.executable,
            str(ROOT / "experiments" / "groundtruth" / "audit_pdf.py"),
            "--cases",
        ...
        for strategy in strategies:
            ...
            materialize_command = [
                sys.executable,
                str(ROOT / "experiments" / "groundtruth" / "materialize.py"),
                "materialize",
                "--cases",
                str(args.cases.resolve()),
                "--chunks",
        ...
        if args.plot_results:
            plot_command = [
                sys.executable,
                str(ROOT / "experiments" / "plot_pdf_ir.py"),
                "--run-dir",
                str(run_dir),
                "--bootstrap-repetitions",
                str(args.bootstrap_repetitions),
