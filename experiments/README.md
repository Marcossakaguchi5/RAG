...
- Ponto de entrada único para novas rodadas: `experiments/run_experiment.py`.
- SciQ e gráficos: `ingest/api/benchmarks/sciq/run_all.py`.
- Exportação dos chunks do Qdrant:
  `ingest/api/benchmarks/groundtruth/export_chunks.py`.
- Esquema mestre e materialização: [groundtruth/README.md](groundtruth/README.md).
- RI em PDFs: `ingest/api/benchmarks/groundtruth/run_groundtruth.py`.
- Coleta e RAGAS offline: `chat/api/benchmarks/ragas/`.
...
pip install -r ingest/api/requirements.txt
pip install -r ingest/api/benchmarks/sciq/requirements-benchmark.txt
```
...
python experiments/groundtruth/audit_pdf.py \
...
python experiments/run_experiment.py rag \
  --cases experiments/runs/novas/pdf-ir/ID_DA_RODADA/recursive_text/ragas-groundtruth.jsonl \
